var tabs = (function(popupModule) {
  ...
  Tab.prototype.initLabelContainer = function() {
    ...
    this.setLabel('Loading...'); // Loading text

    closeLink.href = '#close-' + name;
    closeLink.innerHTML = '&#10005;'; // Close icon - &#10006;
  ...
  // This involves the 'title.js' file which I do not have access to by the nature of the Chrome Inspect Extension tools.
  // Because of this, the tab title settings are slightly messed up and have been done BELOW this comment
  // (if the following lines are not commented out).
  Tab.prototype.doLoadStop = function(e) {
    ...
    if (!this.scriptInjectionAttempted) {
      ...
      /*try {
        ...
        document.getElementById('status').innerText = 'Status: err on set title/label';
      }*/

      // Set label to URL
      //this.setLabel('Tab label'); // testing label (DBG)
      this.setLabel(this.url.substr(8, 20));
  ...
  Tab.prototype.doScriptInjected = function(results) {
    if (!results || !results.length) {
      ...
      // Unknown purpose of this function
    }
  }*/
